...
- **Deep Mode 기본** — 모든 하네스 구축은 Ouroboros 방법론 기반 심층 도메인 분석을 거침 (소크라테스식 질문 → 모호성 점수 → Seed 명세 → 생성). 생성된 하네스도 실행 시 Deep Mode를 먼저 거침
...
**모든 하네스 구축과 실행은 Deep Mode가 기본이다:**

| 단계 | 하네스 메타 스킬 (이것) | 생성된 하네스 스킬 |
|------|----------------------|------------------|
| 하네스 구축 시 | Phase 1: Deep Mode 인터뷰 → Seed → 생성 | — |
| 생성된 스킬 실행 시 | — | Phase D: Deep Mode 인터뷰 → Seed → 작업 실행 |

즉, 이 스킬로 하네스를 **구축**할 때도, 구축된 하네스로 **작업**을 실행할 때도 항상 소크라테스식 인터뷰 → 모호성 점수 → Seed 과정을 거친다.
...
**모호성 점수**가 임계값을 통과해야만 하네스 생성이 진행된다:
