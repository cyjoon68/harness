...
- **Deep Mode 기본** — 하네스 구축(메타 스킬)은 Ouroboros 방법론 기반 심층 도메인 분석 필수. 생성된 하네스는 Deep Mode 선택 옵션
...
|------|----------------------|------------------|
| 하네스 구축 시 | Phase 1: Deep Mode 인터뷰 → Seed → 생성 **(필수)** | — |
| 생성된 스킬 실행 시 | — | Phase D: Deep Mode 인터뷰 → Seed → 작업 실행 **(선택)** |

즉, 이 스킬로 하네스를 **구축**할 때는 반드시 딥 모드를 거친다. 구축된 하네스로 **작업**을 실행할 때는 필요 시에만 딥 모드를 사용한다.
