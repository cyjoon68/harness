...
- **Deep Mode 기본** — 모든 하네스 구축은 Ouroboros 방법론 기반 심층 도메인 분석을 거침 (소크라테스식 질문 → 모호성 점수 → Seed 명세 → 생성)
...
OpenCode에서 다음과 같이 트리거한다 (모든 하네스 구축은 딥 모드로 동작):
...
Phase 0: 현황 감사 (기존 하네스 확인)
   │
   ▼
Phase 1: Deep Mode — Ouroboros 인터뷰
   │          ├── 소크라테스식 질문
