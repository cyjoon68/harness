...
2. **Deep Mode 기본**: 하네스 구축 시 반드시 Ouroboros 딥 모드(인터뷰 → 모호성 점수 → Seed)를 거친다. 생성된 하네스의 Deep Mode는 선택 옵션이다.
...
3. 현황에 따라 분기:
   - **신규 구축**: 에이전트/스킬 없음 → Phase 1(딥 모드)부터 전체 실행
   - **기존 확장**: 기존 하네스 존재 + 추가 요청 → 영향받는 Phase만 실행
   - **운영/유지보수**: 감사/수정/동기화 요청 → Phase 7로 이동

---

### Phase 1: Deep Mode — Ouroboros Domain Interview

하네스를 구축할 때는 **반드시** 이 Phase를 거친다. Ouroboros 방법론으로 도메인을 심층 분석하여
숨겨진 요구사항을 발굴하고, 모호성을 제거한 후에야 다음 Phase로 진행한다.
...
- [ ] **오케스트레이터에 Phase D (Deep Mode 인터뷰) 포함 (선택)** — 사용자 요청 시 또는 작업 모호 시만 실행
