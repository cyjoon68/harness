...
**하네스 메타 스킬과 생성된 하네스 모두 Deep Mode(인터뷰 → Seed)가 기본이다.**

**핵심 원칙:**
1. **전역 스킬 → 로컬 하네스**: 이 스킬(전역)이 하네스(프로젝트 로컬)를 생성한다.
2. **Deep Mode 기본**: 하네스 구축 시 반드시 Ouroboros 딥 모드(인터뷰 → 모호성 점수 → Seed)를 거친다. 생성된 하네스도 마찬가지로 실행 시 딥 모드를 먼저 거친다.
...
**오케스트레이터에 반드시 포함할 사항:**
- **Phase D: Deep Mode — Domain Interview** — 모든 실행의 첫 단계. 소크라테스식 질문 → 모호성 점수 → Seed 확정.
- Phase D는 기존 컨텍스트가 없거나(초기 실행) 새 입력이 들어왔을 때(새 실행) 반드시 실행.
- `_workspace/` 존재 + 부분 수정 요청(재실행) 시에만 Phase D를 건너뛰고 이전 Seed 재사용.
...
- [ ] **오케스트레이터에 Phase D (Deep Mode 인터뷰) 포함** — 소크라테스식 질문 + 모호성 점수 + Seed
